...
DEADLINE <- as.POSIXct("2026-04-02 23:59:59", tz = "America/Toronto")
...
drivefiles <- drive_ls(DATA_FOLDER) %>%
  ...
  ungroup() %>%
  filter(as.POSIXct(time, format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC") <= DEADLINE) %>%
  group_by(gname) %>%
  slice_max(time) %>%
  ungroup() %>%
  select(name, id, time)
...
lboard <- function(x) {
  ...
  if (is.null(data)) {
    return(make_error("Could not read submission file. File may be corrupt or not a valid CSV."))
  }
...
issues_results <- all_results %>%
  ...
  mutate(team = if_else(!is.na(groupname), groupname, word(filename, 1, sep = "[ .]"))) %>%
  select(team, issue)
...
# Winners by Year Level
winners <- valid_results %>%
  arrange(year, rmse) %>%
  mutate(year_label = if_else(year != "MABE", paste0("Year ", year), year)) %>%
  group_by(year) %>%
  slice_min(rmse, n = 1) %>%
  ungroup() %>%
  select(year_label, groupname, rmse)

winners %>%
  gt() %>%
  tab_header(
    title = md("**Winners by Year Level**"),
    subtitle = "SMITH Competition 2025-26"
  ) %>%
  cols_label(
    year_label = "Year Level",
    groupname  = "Winning Team",
    rmse       = "RMSE"
  ) %>%
